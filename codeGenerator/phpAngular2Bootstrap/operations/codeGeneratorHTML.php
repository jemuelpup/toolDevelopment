...
class CodeGeneratorHTML{
	...
	public function generateInsertForm($dataArray,$tableName){
		...
		<div class="add'.ucfirst($tableName).'">
			<h3>Add '.$tableName.'</h3>
			<form ng-submit="addNew'.ucfirst($tableName).'()">
			...
			<div id="edit'.ucfirst($tableName).'" class="modal edit'.ucfirst($tableName).'">
				<form ng-submit="edit'.ucfirst($tableName).'()">
					...
					<div class="modal-footer">
		        		<button class="waves-effect waves-light btn modal-action modal-close" type="submit">Update</button>
		        	</div>
				</form>
			</div>
		';
	...
	public function generateTableData($dataArray,$tableName){
		...
		<div class="'.$tableName.'List">
			...
			<input ng-model="'.$tableName.'Filter" type="text" placeholder="'.ucfirst($tableName).' filter">
			<div class="data-table-container">
				<table class="data-clickable" ng-init="selected'.ucfirst($tableName).'Index=-1">
				    <tbody>
				        <tr>'.$tHead.'</tr>
				        <tr ng-repeat="x in '.$tableName.'s | filter : '.$tableName.'Filter" ng-click="'.$tableName.'Index($index)" ng-class="{\'active\': $index === selected'.ucfirst($tableName).'Index}">'.$tData.'</tr>
				    </tbody>
				</table>
			</div>
			<a class="waves-effect waves-light btn" ng-click="edit'.ucfirst($tableName).'Trigger()">Edit</a>
